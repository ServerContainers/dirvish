...
IMG="servercontainers/dirvish"

PLATFORM="linux/amd64,linux/arm64,linux/arm/v7,linux/arm/v6"

if [ -z ${DEBIAN_VERSION+x} ] || [ -z ${DIRVISH_VERSION+x} ]; then
...
if echo "$@" | grep -v "force" 2>/dev/null >/dev/null; then
  ...
  echo "$@" | grep -v "release" && docker pull "$IMG:d$DEBIAN_VERSION-dv$DIRVISH_VERSION" 2>/dev/null >/dev/null && echo "image already build" && exit 1
fi

docker buildx build -q --pull --no-cache --platform "$PLATFORM" -t "$IMG:d$DEBIAN_VERSION-dv$DIRVISH_VERSION" --push .
