#!/bin/sh
echo ">> copy config"
cp /container/config/dirvish/master.conf /etc/dirvish/master.conf
...
$CRONTIME    root    /bin/bash -c "/etc/dirvish/dirvish-cronjob; . /etc/profile; export MAIL_RECIPIENTS="$MAIL_RECIPIENTS"; /container/scripts/dirvish-mailer.sh"
EOF
